	...
	def _populate_items(self, display_data_dict):
		...
		for key, element in display_data_dict.items():
			if isinstance(element, HasDisplayData):
				subcomponent_display_data = DisplayData(element._namespace(),element.display_data())
			...
			if isinstance(element, DisplayDataItem):
				...
				continue
			self.items.append(DisplayDataItem(element, namespace=self.namespace, key=key))
	@classmethod
...
class DisplayDataItem(object):
	...
	def __init__(self, value, url=None, label=None, namespace=None, key=None, shortValue=None):
		self.namespace = namespace
		self.key = key
