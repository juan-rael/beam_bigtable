...
class DisplayData(object):
  ...
  def addItemIf(self, condition, spec):
    ...
    if spec is None:
      raise ValueError("Input display item cannot be null")
    if spec.get_value() is None:
      raise ValueError("Input display value cannot be null")
    if spec.get_namespace() == None:
      spec = spec.with_namespace(self.namespace)
    item = DisplayDataItem.create(spec, self.path)
    key = DisplayDataItem.ofIdentifier(item.get_path(),item.get_namespace(),item.get_key())
    return self.add(item, key)
  def include(self, path, subComponent):
    if subComponent is None:
      raise ValueError("subComponent argument cannot be null")
    if path is None:
    ...
    return self.include(absolutePath, subComponent)
  @classmethod
  def equals(cls, element,element_compare):
    if isinstance(element, DisplayDataItem):
      return element.get_value() == element_compare
    return False
  @classmethod
  def none(cls):
    return None
class DisplayDataItem(object):
  ...
  def __str__(self):
    return '{}{}:{}={}'.format(self.path, self.namespace.__class__.__name__, self.key, self.value)
  def get_key(self):
    return self.key
  def get_value(self):
    return self.value
  ...
  def _get_value_type(cls, value):
    ...
    return type_
  @classmethod
  def create(cls, spec, path):
    if spec is None:
      raise ValueError("spec cannot be null")
    if path is None:
      raise ValueError("path cannot be null")
    if spec.get_namespace() is None:
      raise ValueError("namespace must be set")
    if spec.get_key() is None:
      raise ValueError("value cannot be null")
    return DisplayDataItem(spec.value,
      url=spec.url,
      label=spec.label,
      namespace=spec.namespace,
      key=spec.namespace,
      shortValue=spec.shortValue) 
  @classmethod
  def ofIdentifier(cls, path, namespace, key):
    if path == None:
      raise ValueError('Null path')
    if namespace == None:
      raise ValueError('Null namespace')
    if key == None:
